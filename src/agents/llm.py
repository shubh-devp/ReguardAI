...
# """Shared Gemini client access and JSON generation utility for agent modules."""
...
# def as_bool(value) -> bool:
#     """Safely convert model response variations into a real boolean."""
#     if isinstance(value, bool):
#         return value
#     if isinstance(value, str):
#         return value.strip().lower() in {"true", "yes", "1"}
...
# def align_by_index(entries, count):
#     """Align batched model response items securely with their designated indices."""
#     aligned = [None] * count
#     if not isinstance(entries, list):
#         return aligned
...
# def _strip_code_fence(text: str) -> str:
#     """Remove markdown code blocks wrapping raw JSON outputs."""
#     text = text.strip()
#     if text.startswith("```json"):
#         text = text[7:]
#     elif text.startswith("```"):
...
# def generate_json(prompt: str, temperature: float = 0.1, response_schema=None) -> dict:
#     """Query Gemini with automatic retries and return the parsed JSON dictionary."""
#     client = get_client()
...
#         except Exception as error:
#             last_error = error
#             print(f"Warning: Gemini call failed (attempt {attempt}/{MAX_ATTEMPTS}): {error}")
#             if attempt < MAX_ATTEMPTS:
#                 time.sleep(2**attempt)
