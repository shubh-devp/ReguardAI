...
- Embeddings run on **ONNX Runtime**, not PyTorch. `all-MiniLM-L6-v2` is the same model either way,
  ...
  hundreds of MB of memory on a 512 MB instance. The ONNX build needs `onnxruntime` only, and the
  model file is committed under `data/models`, so no download happens at startup.
...
- The frontend reads `VITE_API_URL` from `frontend/.env.production`. It must include the full
  `/api/audit` path; posting to the bare origin returns 405 from Flask.
